...
export default function Panels({ saveFile }: Readonly<{ saveFile?: SaveFile }>){
    ...
    return (
        <div className="h-dvh grid grid-rows-5 sm-h:grid-rows-8 gap-3 lg:p-6">
                ...
                <div className="h-full grid gap-6 grid-cols-1 lg:grid-cols-3">
                    ...
                    <div className={`col-span-2 ${activeNarrativePanel !== "narrative" && "hidden lg:block"}`}>
                        <div className="h-full">
                            <div className={`h-[calc(85dvh-0.75rem-1.5rem-0.75rem)] ${BORDER}`}>
                                {saveFile?.saveData && <Narrative saveData={saveFile.saveData} />}
                            </div>
                            <div className="pt-6" />
                            <div className={`h-[calc(15dvh-1.5rem)] overflow-hidden ${BORDER}`}>
                    ...
                    `}>
                        <div className={`row-span-2 lg:row-span-1 ${BORDER} ${activeNarrativePanel !== "party" && "hidden lg:block"}`}>
                            party
                            <div id="partyInners"></div>
                        </div>
                        <div className={`row-span-2 lg:row-span-1 ${BORDER} ${activeNarrativePanel !== "hq" && "hidden lg:block"}`}>
                            hq
                        </div>
                    </div>
